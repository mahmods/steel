...
Scroller = function( settings ) {
	...
	this.throttle         = false;
	this.handle           = '<div id="infinite-handle" style="display:flex; justify-content:center; margin-top:20px;"><button class="btn primary round-corner">' + text.replace( '\\', '' ) + '</button></div>';
	...
	if ( type == 'scroll' ) {
		...
		if ( this.click_handle ) {
			//this.element.insertAfter( this.handle );
...
Scroller.prototype.refresh = function() {
	...
	if ( this.click_handle ) {
		loader = '<span class="infinite-loader" style="width:28px; margin:15px auto;"></span>';
//		this.element.append( loader );
$(this.element).after(loader);

		loader = this.element.parent().find( '.infinite-loader' );
					...
					if ( response.lastbatch ) {
						...
						if ( self.click_handle ) {
							//self.element.append( self.handle );
                                                        $(self.element).after(self.handle);
						} else {
...
$( document ).ready( function() {
	...
	text  = 'Load More'//infiniteScroll.settings.text;
